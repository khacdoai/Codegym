...
function List() {
    ...
    const [roomToDelete] = useState(null);
    //Phân trang
    ...
    useEffect(() => {
        const fetchMotelroom = async () => {
            try {
                const response = await axios.get("http://localhost:3000/baiHat");
    ...
    const filteredMotelRoom = baiHat.filter(
        (baiHat) =>
            baiHat.tenBaiHat &&
            baiHat.tenBaiHat.toLowerCase().includes(searchTerm.toLowerCase())
    );
            ...
            <div className="table-responsive">
                <div className="table-wrapper">
                    ...
                    <table className="table table-hover">
                        ...
                        <tbody>
                        {currentProducts.map((baiHat, index) => (
                            <tr key={index}>
                                <td>{index + 1}</td>
                                <td>{baiHat.tenBaiHat}</td>
                                <td>{baiHat.caSi}</td>
                                <td>{baiHat.nhacSi}</td>
                                <td>{baiHat.thoiGianPhat}</td>
                                <td>{baiHat.soLuotThich}</td>
                                <td>{baiHat.trangThai}</td>
                                <td>
                                   <button onClick={() => handleShowDetailsModal(baiHat)} >Công Khai </button>
                                </td>
                            </tr>
                        ))}
                        </tbody>
