...
<form onsubmit="return validateFormCreate()" action="/customerServlet?action=create" method="post" style="padding: 20px">
    ...
    <div class="col-6">
        <label for="so_cmnd" class="form-label">Lương: </label>
    ...
    <div class="col-6">
        <p>Vị trí</p>
    <select class="form-select" aria-label="Default select example" id="maViTri" name="maViTri">
        <option value="">Chọn vị trí</option>
        <c:forEach items="${listVitri}" var="vtl">
            <option value="${vtl.maVitri}" >${vtl.tenVitri}</option>
        </c:forEach>
    </select>
    </div>
    <div class="col-6">
        <p>Trình độ</p>
        <select class="form-select" aria-label="Default select example" id="maTrinhDo" name="maTrinhDo">
            <option value="">Chọn trình độ</option>
            <c:forEach items="${listTrinhDo}" var="tdl">
                <option value="${tdl.maTrinhDo}" >${tdl.tenTrinhDo}</option>
            </c:forEach>
        </select>
    </div>
    <div class="col-6">
        <p>Bộ phận</p>
        <select class="form-select" aria-label="Default select example" id="maBoPhan" name="maBophan">
            <option value="">Chọn bộ phận</option>
            <c:forEach items="${listBoPhan}" var="bpl">
                <option value="${bpl.maBoPhan}" >${bpl.tenBoPhan}</option>
            </c:forEach>
        </select>
    </div>
