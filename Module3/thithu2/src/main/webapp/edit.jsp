...
<fieldset>
    ...
    <form onsubmit="return validateFormCreate()" action="/productServlet?action=edit" method="post" style="padding: 20px">
        <input name="idProduct" id="idProduct" type="hidden" value="${requestScope["productDto"].getId()}">
        <div class="col-6">
            <label for="maMatHang" class="form-label">Mã mặt hàng: </label>
            <input type=text class="form-control" id="maMatHang"  width="90px" name="maMatHang" value="${requestScope["productDto"].getMaMatHang()}">
        </div>
        <div class="col-6">
            <label for="tenMatHang" class="form-label">Tên mặt hàng: </label>
            <input type="text" class="form-control" id="tenMatHang"  name="tenMatHang" value="${requestScope["productDto"].getTenMaHang()}">
        </div>
        <div class="col-6">
            <label for="gia" class="form-label">Giá: </label>
            <input type="text" class="form-control" id="gia" name="gia" value="${requestScope["productDto"].getGia()}">
        </div>
        <div class="col-6">
            <label for="soLuong" class="form-label">Số lượng: </label>
            <input type="text" class="form-control" id="soLuong" name="soLuong" value="${requestScope["productDto"].getSoLuong()}">
        ...
        <div class="col-6">
            ...
            <select class="form-select" aria-label="Default select example" id="nhaSanXuat" name="nhaSanXuat">
                ...
                <c:forEach items="${nhaSanXuatList}" var="nsx">
                    <option value="${nsx.getIdNhaSanXuat()}"
                    ${nsx.getIdNhaSanXuat() == requestScope["productDto"].getIdNhaSanXuat() ? "selected" : ""}>${nsx.getTenNhaSanXuat()}</option>
                </c:forEach>
            </select>
        </div>
