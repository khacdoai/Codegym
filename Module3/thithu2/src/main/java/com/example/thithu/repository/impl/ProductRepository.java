...
public class ProductRepository implements IProductRepository {
    ...
    private static final  String SELECTER ="SELECT p.*, lmh.ten_loai_mat_hang, nsx.ten_nha_san_xat FROM product p\n" +
            "             join nha_san_xuat nsx on p.id_nha_san_xuat = nsx.id_nha_san_xuat\n" +
            "\t\t\tjoin loai_mat_hang lmh on p.id_lo    ai_mat_hang = lmh.id_loai_mat_hang\n" +
    ...
    private static final String SEARCH = "SELECT p.*, lmh.ten_loai_mat_hang, nsx.ten_nha_san_xat FROM product p\n" +
            "            join nha_san_xuat nsx on p.id_nha_san_xuat = nsx.id_nha_san_xuat\n" +
            "            join loai_mat_hang lmh on p.id_loai_mat_hang = lmh.id_loai_mat_hang\n" +
            "            where ten_mat_hang like concat('%',?,'%') and p.trang_thai = 0;";
    private static final String SEARCH_BY_MA_MAT_HANG = "SELECT p.*, lmh.ten_loai_mat_hang, nsx.ten_nha_san_xat FROM product p\n" +
            "                   join nha_san_xuat nsx on p.id_nha_san_xuat = nsx.id_nha_san_xuat\n" +
            "                     join loai_mat_hang lmh on p.id_loai_mat_hang = lmh.id_loai_mat_hang\n" +
            "                     where ma_mat_hang like concat('%',?,'%') and p.trang_thai = 0;";
    @Override
    ...
    public List<ProductDto> searchProductByMaMatHang(String maMatHang) {
        ...
        Connection connection = BaseRepository.getConnection();
        try {
            PreparedStatement preparedStatement = connection.prepareStatement(SEARCH_BY_MA_MAT_HANG);
            preparedStatement.setString(1,maMatHang);
            ResultSet resultSet = preparedStatement.executeQuery();
            while (resultSet.next()){
                int id = resultSet.getInt("id");
                String maMatHang1 = resultSet.getString("ma_mat_hang");
                String tenMaHang = resultSet.getString("ten_mat_hang");
                float gia = resultSet.getFloat("gia");
                int soLuong = resultSet.getInt("so_luong");
                int idLoaiMatHang = resultSet.getInt("id_loai_mat_hang");
                ...
                String tenLoaiMatHang = resultSet.getString("ten_loai_mat_hang");
                String tenNhaSanXuat = resultSet.getString("ten_nha_san_xat");
                list.add(new ProductDto(id,maMatHang1,tenMaHang,gia,soLuong,idLoaiMatHang,idNhaSanXuat,tenLoaiMatHang,tenNhaSanXuat));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
